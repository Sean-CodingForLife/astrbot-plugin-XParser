...
插件可以额外启动一个小型 HTTP 服务，专门给 NapCat 拉取临时媒体。
...
- `send.enable_temp_media_http_fallback` 是唯一 HTTP 总开关

也就是说：

- 开启 `send.enable_temp_media_http_fallback`
  - 插件会启动内置临时媒体 HTTP 服务
  - 媒体发送时会尝试 HTTP 临时 URL 这一层
- 关闭 `send.enable_temp_media_http_fallback`
  - 插件不会启动临时媒体 HTTP 服务
...
前两层都不行时，才会继续进入本地视频消息 / Stream API / 文件回退链路。
