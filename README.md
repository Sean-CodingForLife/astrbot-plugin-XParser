...
发送模式选择为 `merged` 后，图片推文会尽量按下面的形式发送：

```text
推文文字摘要 + 图片 1 + 图片 2 + ...
```

发送模式选择为 `forward` 后，插件会尝试发送 QQ 合并聊天记录：
...
视频仍会单独发送，因为 QQ/OneBot 对“文字 + 图片 + 视频”混合消息以及合并转发内的视频节点兼容性较差。如果合并转发发送失败，插件会自动退回图文合并模式。
...
```text
send_mode = normal | merged | forward
```

| 模式 | 显示效果 | 说明 |
|---|---|---|
| `normal` | 普通发送 | 先发推文文字，再逐张发送图片，视频单独发送。兼容性最好，但消息数量较多。 |
| `merged` | 图文合并 | 尽量把推文文字和图片放到一条普通消息里，视频仍单独发送。默认推荐。 |
| `forward` | 合并转发 | 尝试发送 QQ 合并聊天记录，文字和图片作为多个节点展示；视频仍单独发送。失败时自动退回 `merged`。 |

旧配置项 `merge_text_and_images` 仍然保留用于兼容老版本。如果 `send_mode` 没有填写，插件会根据旧配置自动判断：开启时等同 `merged`，关闭时等同 `normal`。
